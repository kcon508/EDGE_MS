...
```{r Figure 3b - 2012 vs LTAs, include=TRUE}
#### ANOVAs 2012 vs 9-yrs and 2013 vs. 9 yrs ***Table S7*** ####
str(ANPP_PPT)
...
## Drop KNZ ##
three.sensitivity<-lmer((STATS_g_m2_mm)~Site*nat_exp+(1|Block.ID),data=subset(redu.12vs14,Site!="KNZ"))
anova(three.sensitivity,ddf="Kenward-Roger")
pairs(emmeans(three.sensitivity,pairwise~nat_exp|Site))

three.sensitivity<-lmer((STATS_g_m2_mm)~Site+nat_exp+(1|Block.ID),data=subset(redu.12vs14,Site!="KNZ"))
anova(three.sensitivity,ddf="Kenward-Roger")
pairs(emmeans(three.sensitivity,pairwise~nat_exp))

## Check KNZ alone ##
KNZ.sen.12vs14.NAT<-subset(redu.12vs14,Site=="KNZ" & nat_exp=="natural")
KNZ.sen.12vs14.EXP<-subset(redu.12vs14,Site=="KNZ" & nat_exp=="experimental")

t.test(KNZ.sen.12vs14.NAT$STATS_g_m2_mm,KNZ.sen.12vs14.EXP$STATS_g_m2_mm)
...
## Drop KNZ ##
three.sensitivity<-lmer(STATS_g_m2_mm~Site+nat_exp+(1|Block.ID),data=subset(ANPP_PPT_12vs14to17,Site!="KNZ"))
anova(three.sensitivity,ddf="Kenward-Roger")
pairs(emmeans(three.sensitivity,pairwise~nat_exp))

## Check KNZ alone ##
KNZ.sen.12vs14to17.NAT<-subset(ANPP_PPT_12vs14to17,Site=="KNZ" & nat_exp=="natural")
KNZ.sen.12vs14to17.EXP<-subset(ANPP_PPT_12vs14to17,Site=="KNZ" & nat_exp=="experimental")

t.test(KNZ.sen.12vs14to17.NAT$STATS_g_m2_mm,KNZ.sen.12vs14to17.EXP$STATS_g_m2_mm)
```
...
KNZ_no13<-subset(ANPP_9_LTAs_no_13,Site=="KNZ")
HYS_no13<-subset(ANPP_9_LTAs_no_13,Site=="HYS")
CHY_no13<-subset(ANPP_9_LTAs_no_13,Site=="CHY")
SGS_no13<-subset(ANPP_9_LTAs_no_13,Site=="SGS")

KNZ_2013<-subset(ANPP_PPT,Year=="2013" & Site=="KNZ")
HYS_2013<-subset(ANPP_PPT,Year=="2013" & Site=="HYS")
CHY_2013<-subset(ANPP_PPT,Year=="2013" & Site=="CHY")
SGS_2013<-subset(ANPP_PPT,Year=="2013" & Site=="SGS")

#### T-tests 2013 vs. LTAs - **Fig. 5 / Table S7** ####
t.test(KNZ_2013$ANPP,mu=KNZ_no13$ANPP.LTA)
t.test(HYS_2013$ANPP,mu=HYS_no13$ANPP.LTA)
t.test(CHY_2013$ANPP,mu=CHY_no13$ANPP.LTA)
t.test(SGS_2013$ANPP,mu=SGS_no13$ANPP.LTA)
...
rec.mod<-lm(log(ANPP)~Site*comb_TRT_by_period,data=ANPP_PPT.2018)
...
plot(rec.mod2)
qqnorm(resid(rec.mod2));qqline(resid(rec.mod2))

anova(rec.mod2,ddf="Kenward-Roger")
pairs(emmeans(rec.mod2,pairwise~comb_TRT_by_period|Site))


rec.mod2<-lm(log(ANPP)~Site*comb_TRT_by_period,data=ANPP_PPT.18to21)
Anova(rec.mod2,type=3)
pairs(emmeans(rec.mod2,pairwise~comb_TRT_by_period|Site))

```

```{r 2012 and 2013 vs. other yrs w/ two sample means, include=TRUE}
# Want to compare 2012 and 2013 ANPP (most 2012) w/ other 9/10-yrs ANPP data rather than do the one-sample t-test w/ 2012 or 2013 as the known value compared against ANPP control average across all other yrs

str(ANPP_PPT)

#### Reassign data periods ####
anpp.12.13.sub<-ANPP_PPT%>%
  filter(comb_TRT_by_period=="con")%>%
  mutate(period_12=case_when(
    Year %in% c("2012") ~ "2012",
    Year %in% c(2013:2021) ~ "comp_yrs"))%>%
  mutate(period_13=case_when(
    Year %in% c("2013") ~ "2013",
    Year %in% c(2012,2014:2021) ~ "comp_yrs"))

# view(anpp.12.13.sub)

# dat.export<-ANPP_PPT%>%
#   select(Site,Year,Block,comb_TRT_by_period,Plot,ANPP,Block.ID,Plot.ID)%>%
#   mutate(Site=case_when(
#     Site == "CHY" ~ "HPG",
#     Site %in% c("KNZ","HYS","SGS") ~ Site))
# 
# write.csv(dat.export,"Oecologia_ANPP_PPT_TableS7.csv")

#### 2012 summary stats ####
test.12<-anpp.12.13.sub%>%
  group_by(Site,period_12)%>%
  dplyr::summarise(
    n=n(),
    av=mean(ANPP),
    sd=sd(ANPP),
...
#### 2013 summary stats ####
test.13<-anpp.12.13.sub%>%
  group_by(Site,period_13)%>%
  dplyr::summarise(
    n=n(),
    av=mean(ANPP),
    sd=sd(ANPP),
...
#### Stats - Testing 2012 and 2013 vs. 9-yrs control plots ANPP ####
# 2012 #
mod.12<-lmer(log(ANPP)~Site*period_12+Block.ID+(1|Plot.ID)+(1|Plot.ID:period_12),data=anpp.12.13.sub)
mod.12<-lmer(log(ANPP)~Site*period_12+(1|Plot.ID),data=anpp.12.13.sub)
plot(mod.12)
qqnorm(resid(mod.12));qqline(resid(mod.12))

# mod.12.noKNZ<-lmer(log(ANPP)~Site*period_12+(1|Plot.ID),data=subset(anpp.12.13.sub,Site!="KNZ"))
# anova(mod.12.noKNZ,ddf="Kenward-Roger")
# pairs(emmeans(mod.12.noKNZ,pairwise~period_12|Site))
...
# 2013 #
mod.13<-lmer(log(ANPP)~Site*period_13+Block.ID+(1|Plot.ID)+(1|Plot.ID:period_13),data=anpp.12.13.sub)
mod.13<-lmer(log(ANPP)~Site*period_13+(1|Block.ID)+(1|Block.ID:Plot.ID),data=anpp.12.13.sub)

mod.13<-lmer(log(ANPP)~Site*period_13+(1|Plot.ID),data=anpp.12.13.sub)
plot(mod.13)
qqnorm(resid(mod.13));qqline(resid(mod.13))

# mod.13.noKNZ<-lmer(log(ANPP)~Site*period_13+(1|Plot.ID),data=subset(anpp.12.13.sub,Site!="KNZ"))
# anova(mod.13.noKNZ,ddf="Kenward-Roger")
# pairs(emmeans(mod.13,pairwise~period_13|Site))
...
ANPP_PPT_adj<-ANPP_PPT%>%
  mutate(period_13=ifelse(Year=="2013","2013","comp_yrs"))%>%
  mutate(period_12=ifelse(Year=="2012","2012","comp_yrs"))

avs_adj<-ANPP_PPT_adj%>%
  filter(comb_TRT_by_period=="con")%>%
  group_by(Site,period_13)%>%
  dplyr::summarise(
    n.plots.over.yrs=n(),
